...
function getNames(presidentsArr) {
  const presidentNamesArr = presidentsArr.map(elm => elm.name)
...
function getDemocraticPresidents(presidentsArr) {
  const filteredPresidents = presidentsArr.filter(elm => elm.party === 'Democratic')
...
function countYearsInOffice(presidentsArr) {
  ...
  const sumYears = presidentsArr.reduce((acc, elm) => {

    if (!elm.leftOffice) {
      return acc
    }

    const totalYears = elm.leftOffice - elm.tookOffice
